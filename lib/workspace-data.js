...
const gitLogLines = () => {
  try {
    const log = execSync('git log -5 --pretty=format:%h|%s', { cwd: projectRoot }).toString().trim();
    return log.split('\n').map((line, index) => {
...
const gitStatusLines = () => {
  try {
    const output = execSync('git status -sb', { cwd: projectRoot }).toString().trim();
...
const gatherSkills = () => {
  ...
  const entries = listDir(skillsDir).filter((entry) => entry.isDirectory()).slice(0, 6);
  if (entries.length === 0) {
    return [
      { name: 'kid-insights', rating: 4.9, desc: 'Monitor Kid core signals', downloads: '6.4K', updated: '1d ago', icon: 'code' },
      { name: 'vibe-ml', rating: 4.6, desc: 'Generative timeline prompts', downloads: '3.2K', updated: '3d ago', icon: 'message' }
    ];
  }
  return entries.map((entry, index) => ({
    name: entry.name,
    rating: (4 + index * 0.1).toFixed(1),
    desc: 'Local skill ready for quick installs.',
    downloads: `${Math.floor(2 + index * 1.5)}K`,
    updated: `${index + 1}d ago`,
    icon: ['code', 'globe', 'mail', 'calendar', 'message', 'database'][index % 6]
  }));
};
...
export function buildWorkspaceData() {
  ...
  const heroMetrics = [
    ...
    { label: 'Skills ready', value: `${gatherSkills().length}`, trend: '+1', type: 'up' },
    { label: 'Git commits', value: execSync('git rev-list --count HEAD', { cwd: projectRoot }).toString().trim(), trend: '+1', type: 'up' },
  ...
  return {
    ...
    health: {
      ...
      liveActivity: liveEvents,
      recentLogs: liveEvents.slice(0, 6).map((e, idx) => ({ time: `${16 - idx}:${(idx * 3).toString().padStart(2, '0')}`, type: 'action', detail: e.text, duration: `${idx + 1}s` }))
    },
    skills: gatherSkills(),
    memory: {
